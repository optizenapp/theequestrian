...
export default function UncategorizedArticlesPage() {
  ...
  const getRelevantCategories = (articleType: string) => {
    const categoryMap: Record<string, string[]> = {
      news: ['Sport', 'Business', 'Weather', 'Community', 'Visitor Economy News'],
      inspiration: ['Food & Drink', 'Arts & Culture', 'Features', 'Seasonal', 'Outdoors'],
      history: ['Heritage', 'Genealogy', 'Churches', 'Industrial', 'Archaeology'],
  ...
  return (
    <AdminLayout title="Uncategorized Articles" subtitle="Assign categories to articles">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-1">Review and assign categories to these articles</p>
          </div>
          <Link
            href="/admin/articles"
            className="px-4 py-2 bg-action text-white rounded-lg hover:opacity-90 transition-colors font-bold text-sm"
          >
            Back to Articles
          </Link>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-lg shadow p-8 text-center text-gray-500">
            ...
            <p className="text-sm mt-2">No uncategorized articles remaining.</p>
          </div>
        ) : (
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {articles.map((article) => {
                  ...
                  const viewUrl = getArticleUrl({ slug: article.slug });
                  return (
                    <tr key={article.article_id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        <select
                          ...
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-action focus:border-transparent text-sm disabled:opacity-50"
                        >
                        ...
                        {updating === article.article_id && <p className="text-xs text-gray-500 mt-1">Updating...</p>}
                      </td>
