#!/usr/bin/env tsx
/**
 * Reallocate /accessories/gifts products whose product_type maps to new leaves.
 * Writes PCA updates + PDP 301 manual_redirects.
 *
 *   npx tsx scripts/reallocate-accessories-gifts-split.ts --floral-prod
 *   npx tsx scripts/reallocate-accessories-gifts-split.ts --floral-prod --apply
 */
...
async function main(): Promise<void> {
  ...
  console.log('Reallocate accessories gifts split');
  ...
  for (const row of rows) {
    if (!isGiftsCategoryPath(row.category_path)) continue;
  ...
  console.log(`Candidates on gifts: ${rows.length}`);
  ...
  const logPath = resolve(
    process.cwd(),
    'exports',
    `accessories-gift-split-${dryRun ? 'dry' : 'apply'}-${ts}.csv`
  );
  ...
  console.log('Revalidate via POST /api/internal/revalidate-collection for gifts + new hubs/leaves.');
}
