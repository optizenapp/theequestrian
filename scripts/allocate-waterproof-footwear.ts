#!/usr/bin/env tsx
/**
 * Report candidates for /clothing/footwear/waterproof, then apply approved handles.
 *
 * Report: npx tsx scripts/allocate-waterproof-footwear.ts
 * Apply:  npx tsx scripts/allocate-waterproof-footwear.ts --apply --approved-file exports/waterproof-footwear-approved-handles.txt
...
const TARGET = '/clothing/footwear/waterproof';
const CANDIDATE_CSV = 'exports/waterproof-footwear-candidates.csv';
...
async function main() {
  ...
  const rows = await sql`
    ...
    WHERE (pca.category_path = '/clothing/footwear' OR pca.category_path LIKE '/clothing/footwear/%')
      AND (
        lower(p.title) LIKE '%waterproof%'
        OR lower(coalesce(p.product_type, '')) LIKE '%waterproof%'
        OR EXISTS (
          SELECT 1 FROM unnest(p.tags) AS tag WHERE lower(tag) LIKE '%waterproof%'
        )
      )
      AND NOT (
        lower(p.title) LIKE '%motorcycle%'
        OR lower(p.title) LIKE '%motorbike%'
        OR lower(p.title) LIKE '%atv riding%'
        OR lower(p.title) LIKE '%bike riding%'
      )
  ...
  console.log('Review, copy handles to exports/waterproof-footwear-approved-handles.txt, then:');
  console.log(
    '  npx tsx scripts/allocate-waterproof-footwear.ts --apply --approved-file exports/waterproof-footwear-approved-handles.txt'
  );
}
