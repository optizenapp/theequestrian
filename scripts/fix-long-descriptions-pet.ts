...
/**
 * Fix Long Descriptions - Pet Pages
 * 
 * Updates only the long descriptions for pet pages with detailed,
 * category-specific content focused on pet care and wellbeing.
 * 
 * Usage:
 *   npm run fix-long-desc-pet -- --dry-run
 *   npm run fix-long-desc-pet
 */
...
async function processPetPage(row: CsvRow, allRows: CsvRow[], dryRun: boolean): Promise<boolean> {
  ...
  const newLongDesc = generateLongDescription(row, allRows);
  
  // Check if current description is generic/template
  const isGeneric = oldLongDesc.includes('Discover our comprehensive collection') || 
                    oldLongDesc.includes('carefully curated to meet the needs') ||
                    oldLongDesc.length < 300;
  
  if (!isGeneric && oldLongDesc.length > 600) {
...
async function main() {
  ...
  if (!dryRun) {
    ...
    const backupPath = CSV_PATH.replace('.csv', `-backup-long-desc-pet-${timestamp}.csv`);
